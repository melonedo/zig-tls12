    ...
    if (potential_proxy) |proxy| {
        ...
        if (proxy.supports_connect) tunnel: {
            ...
            if (protocol == .tls) {
                ...
                conn.protocol = .tls;
                conn.tls_client = try client.allocator.create(TlsClient);
                errdefer client.allocator.destroy(conn.tls_client);

                conn.tls_client.* = TlsClient.init(conn.stream, client.ca_bundle, host) catch return error.TlsInitializationFailed;
                // This is appropriate for HTTPS because the HTTP headers contain
                // the content length which is used to detect truncation attacks.
